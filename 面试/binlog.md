...
salve 端有两个线程 `IO线程` 和 `SQL线程` 

当 master 端更新了 binlog 后会通知 salve 端，salve 端的 `IO线程` 就会去 master 端去读取 binlog 并写入到 salve 端的中继日志 `relaylog` 中，`SQL线程` 会去读取 `relaylog` 去修改 salve 数据